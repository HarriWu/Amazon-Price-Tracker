...
def compare_price(URL, set_price, user_agent, body):
    """ Compares set price of product to the price scraped off of Amazon
        using BeautifulSoup and returns an updated report.

    Args:
        URL: Amazon URL where price is scrapped.
        set_price: Desired price for that product.
...
def extracting_values(excel_file):
    ...
    while sheet.cell(row=index, column=3).value or sheet.cell(row=index, column=4).value is not None:
        ...
        print(body)
        #It checks everything at random times so that you won't get blocked
        time.sleep(random.randint(10, 20))
        index += 1

    print('done')

    #Send email at the end of the day containing all the links to the products that are under the set price
    if body != '':
